...
Flask single-file (`src/web_app.py`, 5080 строк, 98 роутов, 50 `@admin_required`, 9 `@auth_required`, 76 `@api_error_handler`) + `src/credential_store.py` (124). Три под-приложения: **MV** (`/mediavault/`, read-only), **CM** (`/content-mgmt/`, admin-only), **Admin** (`/admin`).
...
## JS модули (32 файла, 9594 строк без lib/)
...
### Background downloads (`src/web_app.py`)
- `_download_tasks` dict + `threading.Lock` — очередь фоновых загрузок
- `_start_background_task(type, fn, *args)` — запускает daemon thread
- `GET /api/content-search/task/<task_id>` — поллинг статуса (running/completed/failed + progress/total)
- `POST /api/content-search/download-async` — одительный файл
- `POST /api/content-search/download-manga-async` — NHentai манга (ThreadPoolExecutor 4 workers)
- `GET /api/content-search/check-manga-dir?gid=...&media_id=...&title=...` — проверка существования папки
- `overwrite=True` удаляет файлы перед перезагрузкой

## CSS (8 файлов, 2442 строк)

| Файл | Строк | Что |
|------|-------|------|
| `shared.css` | 281 | CSS vars, темы, base, header, fonts |
| `content.css` | 681 | CM: tags, files, comics, picker, drag-to-tag |
| `content-search.css` | 294 | Content search page |
| `admin.css` | 424 | Admin SPA |
| `mediavault.css` | 290 | Gallery, lightbox, sidebar, mobile (последний!) |
| `tagfetch.css` | 248 | Tagfetch panels |
| `settings.css` | 156 | Settings tabs |
...
`test.py` (782 строки) — кастомный CLI-раннер, **не pytest**:
